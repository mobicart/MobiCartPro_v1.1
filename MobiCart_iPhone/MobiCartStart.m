...
- (void)fetchDataFromServer
{
	...
	else
	{
		...
		dictAppDetails =[GlobalPreferences getDictAppStoreUser];
		...
		else if (dictAppDetails)
		{
			...
			NSDictionary *dictFeatures = [GlobalPreferences getDictFeatures];			
            // Fetch Color schemes
			NSDictionary *dictColorSchemes =[GlobalPreferences getDictColorScheme];
			
			NSDictionary *dictAppVitals = [GlobalPreferences getDictVitals];
...
- (void)fetchData
{
	...
	_objMobicartAppDelegate.arrAllData =(NSArray *)[GlobalPreferences getDictStaticPages];
