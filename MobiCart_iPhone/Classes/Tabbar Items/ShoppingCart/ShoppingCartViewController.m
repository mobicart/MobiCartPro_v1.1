...
- (void)btnQuantity_Clicked:(id)sender
{
    ...
    for(NSInteger i = 0; i < [arrQuantity count]; i++){
        NSString *string = [arrQuantity objectAtIndex:i];
        if([string isEqualToString:lblTemp.text]){
            pickerRow = i;
            break;  //Once we have it break out of the loop
        }
    }
    
    [pickerViewQuantity selectRow:pickerRow inComponent:0 animated:NO];
